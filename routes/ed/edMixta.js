...
router.get('/Resultados', async(req, res, next) =>{
  ...
  const Planteles = await pool.query("SELECT * FROM `ed_general_mix_c22`");
  const Jornadas = await pool.query("SELECT Jornada FROM `ed_jor_mix_c2022` GROUP BY Jornada");
...
router.post('/Resultados', async(req, res, next)=> {
  ...
  for (let i = 0; i < 5; i++) {
    if(GF[i] == ""){
      ...
      await pool.query("INSERT INTO mixto_c2022_ed set ?",[Resultados[i][0]])
      await pool.query("INSERT INTO mixto_c2022_ed set ?",[Resultados[i][1]])


    }  
  }
  res.redirect("http://localhost:8082/ED/Mixta/Resultados/Imagenes");
});

router.get('/Resultados/Imagenes', async (req, res, next) => {
  const resul = await pool.query("SELECT * FROM `ed_jor_mix_c2022` ORDER BY ID DESC LIMIT 30;");
...
router.get('/Resultados/Delete/:id', async (req, res, next) => { 
  ...
  await pool.query("DELETE FROM `futbolce_zon58`.`mixto_c2022_ed` WHERE  `ID`= ?;",[regis_delete])
...
router.post('/Planteles', async(req, res, next)=> {
  ...
  for (let i = 0; i < 5; i++) {
    if(ID[i] == ""){
      ...
      await pool.query("INSERT INTO `ed_planteles_mixto_c22` set ?",[Resultados[i][0]])
...
router.get('/Planteles/Imagenes', async(req, res, next)=> {
    ...
    const plantel = await pool.query("SELECT * FROM `futbolce_zon58`.`ed_planteles_mixto_c22_v` ORDER BY `Equipo` DESC");
    ...
    res.render('planteles-img',{plantel ,Liga , categoria});
      ...
      router.get('/Planteles/Imagenes/:plantel', async(req, res, next)=> {
        ...
        console.log(equipo)
       const plantel = await pool.query("SELECT * FROM `futbolce_zon58`.`ed_planteles_mixto_c22_v` WHERE `Nombre_Equipo`=? ORDER BY `ID` ASC" , [equipo]);
