...
router.get('/Resultados', async(req, res, next) =>{
  ...
  const Planteles = await pool.query("SELECT * FROM `g_general_libre_c22`");
  const Jornadas = await pool.query("SELECT Jornada FROM `g_jor_libre_c2022` GROUP BY Jornada");
...
router.post('/Resultados', async(req, res, next)=> {
  ...
  for (let i = 0; i < 5; i++) {
    if(GF[i] == ""){
      ...
      await pool.query("INSERT INTO g_libre_a22 set ?",[Resultados[i][0]])
      await pool.query("INSERT INTO g_libre_a22 set ?",[Resultados[i][1]])
...
router.get('/Resultados/Delete/:id', async (req, res, next) => { 
  ...
  await pool.query("DELETE FROM `futbolce_zon58`.`g_libre_a22` WHERE  `ID`= ?;",[regis_delete])
