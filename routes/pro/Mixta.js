...
router.get('/General', async(req, res, next)=> {
  const vistas = await pool.query("SELECT * FROM `pro_general_mix_c22`");
    ...
    const result = await pool.query("SELECT * FROM `pro_mix_c22_jor_c2022`");
...
router.get('/Resultados', async(req, res, next) =>{
  ...
  const Planteles = await pool.query("SELECT * FROM `pro_general_mix_c22`");
  const Jornadas = await pool.query("SELECT Jornada FROM `pro_mix_c22_jor_c2022` GROUP BY Jornada");
...
router.post('/Resultados', async(req, res, next)=> {
  ...
  for (let i = 0; i < 5; i++) {
    if(GF[i] == ""){
      ...
      await pool.query("INSERT INTO pro_mixto_c22 set ?",[Resultados[i][0]])
      await pool.query("INSERT INTO pro_mixto_c22 set ?",[Resultados[i][1]])


    }  
  }
  res.redirect("http://localhost:8082/Pro/Mixta/Resultados/Imagenes");
});

router.get('/Resultados/Imagenes', async (req, res, next) => {
  const resul = await pool.query("SELECT * FROM `pro_mix_c22_jor_c2022` ORDER BY ID DESC LIMIT 30;");
...
router.get('/Resultados/Delete/:id', async (req, res, next) => { 
  ...
  await pool.query("DELETE FROM `futbolce_zon58`.`pro_mixto_c22` WHERE  `ID`= ?;",[regis_delete])
