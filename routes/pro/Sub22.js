...
let fondo = 'url("/images/fondochampions.png")';
...
router.get('/Resultados', async(req, res, next) =>{
  ...
  const Planteles = await pool.query("SELECT * FROM `agui_general_sub22_c22`");
  const Jornadas = await pool.query("SELECT Jornada FROM `agui_jor_sub22_c22` GROUP BY Jornada");
...
router.post('/Resultados', async(req, res, next)=> {
  ...
  for (let i = 0; i < 5; i++) {
    if(GF[i] == ""){
      ...
      await pool.query("INSERT INTO agui_sub22_c22 set ?",[Resultados[i][0]])
      await pool.query("INSERT INTO agui_sub22_c22 set ?",[Resultados[i][1]])


    }  
  }
  res.redirect("http://localhost:8082/Aguigol/Sub22/Resultados/Imagenes");
});
...
router.get('/Resultados/Delete/:id', async (req, res, next) => { 
  ...
  await pool.query("DELETE FROM `futbolce_zon58`.`agui_sub22_c22` WHERE  `ID`= ?;",[regis_delete])
